...
export class Scenes {
    ...
    private _activeScenes: Map<number, Scene> = new Map();
    ...
    static async stageScene(id: (number | SceneReference)[] | number | SceneReference, priority?: number, transitionTimeMs?: number, brightness?: number) {
        ...
        for (const i of id) {
            ...
            this.instance._activeScenes.set(sceneId, scene);
        }
    }
    ...
    static async unstageScene(id: (number | SceneReference)[] | number | SceneReference) {
        ...
        for (const i of id) {
            ...
            this.instance._activeScenes.delete(sceneId);
        }
    }
    ...
    static async sendScenes(transitionTimeMs?: number, brightness?: number) {
        ...
        if (this.instance._activeScenes.size == 0) {
        ...
        const addTarget = (scene: Scene, target: Target, attributes: LightStateAttributes) => {
            ...
            if (!actions.get(target.type)?.has(target)) { actions.get(target.type)?.set(target, newAttributes); }
        }


        //Sort the scenes by priority
        const sortedScenes = Array.from(this.instance._activeScenes.values()).sort((a, b) => {
            if (!a) { return 1; }
            if (!b) { return -1; }
        ...
        for (const scene of sortedScenes) {
            Logger.info(`Applying scene ${scene.name} with priority ${scene.attributes.priority}`);
